...
struct CompactMapView: UIViewRepresentable {
    ...
    /// Optional intermediate waypoints with per-waypoint status colors.
    ...
    final class Coordinator: NSObject, MLNMapViewDelegate {
        ...
        func mapView(_ mapView: MLNMapView, imageFor annotation: MLNAnnotation) -> MLNAnnotationImage? {
            ...
            if let statusRaw = point.subtitle, let status = WaypointStatus(rawValue: statusRaw) {
                // Intermediate waypoint with status color.
            ...
            let image = renderer.image { ctx in
                color.setFill()
                if isSmall {
                    // Smaller diamond marker for intermediate waypoints.
                    ...
                    // Standard pin marker for start/destination.
        ...
        func drawRoute(on mapView: MLNMapView, start: HarbourOption, destination: HarbourOption) {
            ...
            routeKey = key
            // Die Karte nutzt bewusst die kuratierte lokale Route. Externe Seamark-Graphen
            // koennen je nach Datenlage Start- und Zielpunkte an ungeeignete Knoten heften.
